...
const PUBLIC_PAGES=['login.html','daftar.html','lupa-password.html'];
function currentPage(){const p=location.pathname.split('/').pop();return p===''?'index.html':p}
function requireAuth(){
 const page=currentPage();
 const loggedIn=!!localStorage.getItem('loggedIn');
 if(PUBLIC_PAGES.includes(page)){
  if(loggedIn&&(page==='login.html'||page==='daftar.html')){location.replace('index.html');return false;}
  return true;
 }
...
async function bootstrapApp(){if(!requireAuth())return;if(localStorage.getItem('loggedIn'))await hydrateCloudState();renderAll()}
